...
export class Diff extends Context {
  constructor(ctx) {
    ...
    ctx.actions.add("core:show-remote-diff", showRemoteDiff);
    // ctx.keyboard.add("*","ctrl-shift-l","core:show-current-diff");
    ctx.keyboard.add("*", "ctrl-shift-r", "core:show-remote-diff");
    ...
    diffList = diffPanel.find("#node-dialog-view-diff-diff").editableList({
      ...
      addItem: (container, i, object) => {
        ...
        if (tabForLabel.type === 'tab') {
          ...
          titleSpan.html(ctx._("diff.globalNodes"));
        }
        var flowStats = {
          local: {
            addedCount: 0,
        ...
        if (object.newTab || object.remoteTab) {
          ...
          if (tab.type !== undefined) {
            ...
            $('<span>').html(ctx._("diff.flowProperties")).appendTo(originalNodeDiv);
        ...
        if (localDiff.deleted[tab.id]) {
          ...
          if (localDiff.added[tab.id]) {
            ...
            $('<span class="node-diff-status"></span>').html(ctx._('diff.nodeCount', {
    ...
    wires.forEach(function (p, i) {
      ...
      if (p && p.length > 0) {
        ...
        p.forEach(function (d) {
          ...
          var node = allNodes[d];
          if (node) {
            var def = ctx.nodes.getType(node.type) || {};
    ...
    if (unChanged) {
      ...
      if (localDiff.moved[node.id]) {
        ...
        if (!localDiff.deleted[node.z] && node.z !== localN.z && node.z !== "" && !localDiff.newConfig.all[node.z]) {
          ...
          if (node.z === localN.z) {
            localMovedMessage = ctx._("diff.type.movedFrom", {
              id: (localDiff.currentConfig.all[node.id].z || 'global')
            });
          } else {
            localMovedMessage = ctx._("diff.type.movedTo", {
              id: (localN.z || 'global')
            });
          }
      ...
      if (remoteDiff) {
        if (remoteDiff.moved[node.id]) {
          ...
          if (!remoteDiff.deleted[node.z] && node.z !== remoteN.z && node.z !== "" && !remoteDiff.newConfig.all[node.z]) {
            ...
            if (node.z === remoteN.z) {
              remoteMovedMessage = ctx._("diff.type.movedFrom", {
                id: (remoteDiff.currentConfig.all[node.id].z || 'global')
              });
            } else {
              remoteMovedMessage = ctx._("diff.type.movedTo", {
                id: (remoteN.z || 'global')
              });
            }
  ...
  createNodePropertiesTable(def, node, localNodeObj, remoteNodeObj) {
    ...
    if (localNode) {
      ...
      propertyElements['local.id'] = ctx.utils.createObjectElement(localNode.id).appendTo(element);
    } else {
      localCell.addClass("node-diff-empty");
    }
    if (remoteNode !== undefined) {
      ...
      if (remoteNode) {
        ...
        propertyElements['remote.id'] = ctx.utils.createObjectElement(remoteNode.id).appendTo(element);
      } else {
        remoteCell.addClass("node-diff-empty");
      }
    }
    ...
    if (node.hasOwnProperty('x')) {
      ...
      if (localNode) {
        ...
        element = $('<span class="node-diff-element"></span>').appendTo(localCell);
        propertyElements['local.position'] = ctx.utils.createObjectElement({
          x: localNode.x,
          y: localNode.y
        }, {
          path: "position",
      ...
      if (remoteNode !== undefined) {
        ...
        if (remoteNode) {
          ...
          element = $('<span class="node-diff-element"></span>').appendTo(remoteCell);
          propertyElements['remote.position'] = ctx.utils.createObjectElement({
            x: remoteNode.x,
            y: remoteNode.y
          }, {
            path: "position",
    ...
    var properties = Object.keys(node).filter(function (p) {
    ...
    properties.forEach(function (d) {
      ...
      if (localNode) {
        ...
        element = $('<span class="node-diff-element"></span>').appendTo(localCell);
        propertyElements['local.' + d] = ctx.utils.createObjectElement(localNode[d], {
          path: d,
          exposeApi: true,
          ontoggle: function (path, state) {
      ...
      if (remoteNode !== undefined) {
        ...
        if (remoteNode) {
          ...
          element = $('<span class="node-diff-element"></span>').appendTo(remoteCell);
          propertyElements['remote.' + d] = ctx.utils.createObjectElement(remoteNode[d], {
            path: d,
            exposeApi: true,
            ontoggle: function (path, state) {
  ...
  refreshConflictHeader() {
    ...
    if (conflictCount - resolutionCount === 0) {
      $("#node-diff-toolbar-resolved-conflicts").html('<span class="node-diff-node-added"><span class="node-diff-status"><i class="fa fa-check"></i></span></span> ' + ctx._("diff.unresolvedCount", {
        count: conflictCount - resolutionCount
      }));
    } else {
      $("#node-diff-toolbar-resolved-conflicts").html('<span class="node-diff-node-conflict"><span class="node-diff-status"><i class="fa fa-exclamation"></i></span></span> ' + ctx._("diff.unresolvedCount", {
        count: conflictCount - resolutionCount
      }));
    }
  ...
  getRemoteDiff(callback) {
    $.ajax({
      ...
      success: function (nodes) {
        var localFlow = ctx.nodes.createCompleteNodeSet();
        var originalFlow = ctx.nodes.originalFlow();
  ...
  generateDiff(currentNodes, newNodes) {
    ...
    Object.keys(currentConfig.all).forEach(function (id) {
      var node = ctx.nodes.workspace(id) || ctx.nodes.subflow(id) || ctx.nodes.node(id);
  ...
  mergeDiff(diff) {
    ...
    var historyEvent = {
      t: "replace",
      config: ctx.nodes.createCompleteNodeSet(),
      changed: nodeChangedStates,
      dirty: ctx.nodes.dirty(),
      rev: ctx.nodes.version()
    }

    ctx.history.push(historyEvent);

    ctx.nodes.clear();
    var imported = ctx.nodes.import(newConfig);
    ...
    ctx.nodes.version(remoteDiff.rev);
